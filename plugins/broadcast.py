...
from pyrogram.errors import FloodWait, PeerIdInvalid, UserBlocked
    ...
    try:
        async for msg in session.get_chat_history(session.me.id, limit=20):
            if mess.text and msg.text == mess.text:
                return msg
            if mess_file_id and getattr(msg, "media", None):
                for attr in ("photo", "video", "document", "audio", "voice", "animation", "sticker"):
